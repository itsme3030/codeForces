...
void solve() {  
    ...
    int sm = 0;
    sm = accumulate(oth.begin(), oth.end(), 0ll);
    if (sm != pref[n]) {
        cout << "NO\n";
        return;
    }
    ...
    for(int i = k-1; i >= 0; i--) {
        sm -= oth[i];
        auto it = lower_bound(pref.begin(), pref.end(), sm);
        int dis = distance(pref.begin(), it);
        if (pref[dis] != sm) {
            cout << "NO\n";
            return;
        }
        ...
        intervals.push_back({dis, last});
        last = dis-1;
    }
    cout << "YES\n";
    for(int i = 0; i < intervals.size(); i++) {
        int ff = intervals[i].first, ss = intervals[i].second;
        int mxm = v[ff];
        int ind = ff;
        ...
        while (p1 >= ff || p2 <= ss) {
            ...
            if (ind == ff && ind + 1 <= ss && (v[ind] == v[ind + 1])) {
                while (ind + 1 <= ss && v[ind] == v[ind + 1]) {
                    ind++; tmp++;
                    p1 = ind - 1, p2 = ind + 1;
                }
            } 
