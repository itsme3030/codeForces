...
//       
  ...
  // }  
  int ans = INT_MAX;
  int longsize = 0, longseg = 0, shortsegsize = 0;
  for (int i = 0; i < n; i++) {
      
      int segsize = r[i] - l[i] + 1;
      if (segsize > 1) {
          longsize += segsize;
          longseg++;
      } else {
      ...
      if (longsize >= k) {
          ans = min(ans, r[i] - (longsize - k) + (2*longseg));
          //same situation over here
          // r[i] - (k-longsize)
      } 
      if (longsize + shortsegsize >= k) {
          //assume it's stop from the middle
          //will add for only longsize and then only with shortseg and longsize both ... ]
          //longsize - k
          if(k >= longsize)
          ans = min(ans, r[i] + (2*(longseg+(k-longsize))));
      }
  }
  if (ans == INT_MAX) {
      cout << -1 << "\n";
