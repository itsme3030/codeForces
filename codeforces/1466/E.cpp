...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j <= 60; j++) {
            if ((v[i] >>  j) & 1) {
                tot = ((tot % mod) + (((bits[j] % mod) * ((1ll << j) % mod)) % mod)) % mod;
            }
            
            if ((v[i] >> j) & 1) {
                totmul = ((totmul % mod) + ((((n % mod) * ((1ll << j) % mod)) % mod) % mod)) % mod;
            } else {
                totmul = ((totmul % mod) + ((((bits[j] % mod) * ((1ll << j) % mod)) % mod) % mod)) % mod;                
            }
        ...
        tot %= mod;
        totmul %= mod;
         sm = ((sm % mod) + (((tot % mod) * (totmul % mod)) % mod) % mod) % mod;
