...
void solve() {
    ...
    cout << n-ans << "\n";
    // int dp[n][2];
    //[0] - don't drink currently
    //[1] -  drink currently
    // int sum[n][2];
    //if don't drink then sum
    //if you drink then sum
    ...
    // for (int i = 1; i < n; i++) {
        // if (sum[i-1][0] + v[i] >= 0) {
            ...
            // sum[i][0] = max(sum[i][0], v[i] + sum[i-1][0]);
        // }
        // if (sum[i-1][1] + v[i] >= 0) {
            // dp[i][0] = max(dp[i][0], 1 + dp[i-1][1]);
            // sum[i][1] = max(sum[i][1], v[i] + sum[i-1][1]);
        // }
//         
        // if (sum[i-1][0] >= 0) {
            // dp[i][1] = max(dp[i][1], dp[i-1][0]);
            // sum[i][1] = max(sum[i][1], sum[i-1][0]);
        // }
        // if (sum[i-1][1] >= 0) {
            // dp[i][1] = max(dp[i][1], dp[i-1][1]);
            // sum[i][1] = max(sum[i][1], sum[i-1][1]);
        // }
    // }
