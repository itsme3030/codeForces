...
void solve() {  
  ...
  set<int> fh;
  for (int i = 1; i <= 2*n; i++) {
      if (flg[i] == 0) fh.insert(i);
  }
  // vector<bool> vis(2*n+1, 0);
  // auto binsearch = [&](int tar) -> int {
      // int ans = fh[0];
      // int ind = 0;
      // int lo = 0, hi = fh.size() - 1;
      // while (lo <= hi) {
          // int mid = (lo + hi) >> 1;
          // if (fh[mid] <= tar) {
              // ind = mid;
              // if (!vis[fh[mid]]) ans = fh[mid];
              // lo = mid + 1;
          // } else {
              // hi = mid - 1;
          // }
  ...
  for (int i = n*2-1; i >= 1; i-=2) {
      ...
      // int need = binsearch(ele);
      
      // int ls = have.order_of_key(ele);
      // cout << ls  << " for the  :  " << ele << "\n";
      // ls--;
      // int need = *st.find_by_order(ls);
      
      // cout << need  << " for the  :  " << ele << "\n";
      big[i] = need;
  }
  // return;
  //checking
  ...
  for (int i = 1; i <= 2*n; i++) {
      mp[big[i]] += 1;
      if ((i % 2 && big[i] > big[i+1]) || mp[big[i]] > 1) {
          ok = false;
      }
  }
  if (!ok) {
