...
void solve() {
	...
	//ans is quantity C req
	int div = ((fact[quantity - req] % mod) * (fact[req] % mod)) % mod;
	cout << ((fact[quantity] % mod) * (nt.inverse(div, mod) % mod)) % mod << endl;

	// auto mul = [&](int start, int end) -> int {
	// 	int ans = 1;
	...
	// int multiply = mul(req + 1, quantity);
	// int div = mul(1, req);
	// cout << ((((multiply % mod) * (nt.inverse(div, mod))) % mod)) % mod << "\n";
}
