...
void solve() {
    ...
    priority_queue<pair<int,int>, vector<pair<int,int>>, decltype(comp)> mp(comp);
    for(int i = 0; i < n; i++) mp.push({v[i], i});
    
    int presm = 0;
    for(int i = 0; i < n; i++) {
        pair<int,int> mx = mp.top();
        int ind = mx.second;
        if (ind >= i) { 
            if (v[i] >= mx.first) cout << 0 << " ";
            else {
                if (presm + v[i] >= mx.first) cout << i << " ";
                else cout << i + 1 << " ";
            } 
        } else {
            cout << i << " ";
