...
void solve() {
    ...
    for (auto &x : freq) {
        ...
        mx = max(mx, min(m / curr, freq[curr]) * curr);

        if (freq.find(s_curr) != freq.end()) {
            mx = max(mx, min(m / s_curr, freq[s_curr]) * s_curr);

            if (freq[curr] * curr + freq[s_curr] * s_curr <= m) {
                mx = max(mx, freq[curr] * curr + freq[s_curr] * s_curr);
            } else {
                int ff = min(freq[curr], m / curr);
                int ss = m - ff * curr;
                int ssy = min(freq[s_curr], ss / s_curr);

                int curr_tot = ff*curr + ssy*s_curr;
                int diff = m-curr_tot;

                int rest_y = freq[s_curr] - ssy;
                int add = min({rest_y, ff, (m - curr_tot)});
                mx = max(mx, curr_tot + add);
            }
        }
    }
    cout << mx << "\n";
