...
//  
  ...
  // int mnop = op;
  // int mxop = 1e9;
  // for (int curr = ini; curr >= (ini - 10000); curr--) {
      // int needop = 1e9;
      // int lo = 1, hi = n;
      // while (lo <= hi) {
          // int mid = (lo+hi)>>1;
          // if ((pref[mid] - abs(curr-ini) + (n-mid)*curr) <= k) {
              // lo = mid + 1;
              // needop = min(needop, mnop + abs(curr-ini) + (n-mid));
          // } else {
              // hi = mid - 1;
          // }
      // }
      // mxop = min(needop, mxop); 
  // }
  // cout << mxop << "\n";
